...
instance Print Def where
  prt i e = case e of
    ...
    DStm stm -> prPrec i 0 (concatD [prt 0 stm])
    DTypedef type_ exp -> prPrec i 0 (concatD [doc (showString "typedef"), prt 0 type_, prt 0 exp])
  prtList _ [] = (concatD [])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
instance Print Arg where
  prt i e = case e of
    AType type_ -> prPrec i 0 (concatD [prt 0 type_])
    ATypeId stm -> prPrec i 0 (concatD [prt 0 stm])
    AConst stm -> prPrec i 0 (concatD [doc (showString "const"), prt 0 stm])
  prtList _ [] = (concatD [])
